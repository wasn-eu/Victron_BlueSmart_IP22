...
static void publishHADiscoveryMessage(bool publish) {
    ...
    if (publish) {
        ...
        JSON["name"] = "Victron SmartBlue IP22 " + String(settings.systemID) + " Voltage";
        JSON["unique_id"] = "victron-bluesmart-" + String(settings.systemID) + "-voltage";
        JSON["ic"] = "mdi:rotate-360";
        JSON["stat_t"] = devTopic;
        JSON["val_tpl"] = "{{ value_json."+ String(MQTT_SUBTOPIC_VOLTAGE) +" }}";
        addDeviceDescription(JSON);
        publishJSON(JSON, topicCount, true, false);

        JSON["name"] = "Victron SmartBlue IP22 " + String(settings.systemID) + " Charging Current";
        JSON["unique_id"] = "victron-bluesmart-" + String(settings.systemID)+ "-charging-current";
        JSON["ic"] = "mdi:counter";
        JSON["unit_of_meas"] = "kWh";
        JSON["dev_cla"] = "energy";
        JSON["stat_cla"] = "total_increasing";
        JSON["stat_t"] = devTopic;
        JSON["val_tpl"] = "{{ value_json."+ String(MQTT_SUBTOPIC_CURRENT) +" }}";
        addDeviceDescription(JSON);
